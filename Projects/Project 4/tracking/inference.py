...
def constructBayesNet(gameState: hunters.GameState):
    ...
    obsDomain = []
    for dist in range(maxDist + 1 + MAX_NOISE):
        obsDomain.append(dist)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        factorsGivenEvidence = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variableToEliminate in eliminationOrder:
            factorsContainingVariable = []
            remainingFactors = []

            # Separate the factors based on whether they include the variable to eliminate
            for factor in factorsGivenEvidence:
                if variableToEliminate in factor.variables():
                    factorsContainingVariable.append(factor)
                else:
                    remainingFactors.append(factor)

            # Join all factors that contain the variable to eliminate
            joinedFactorResult = joinFactorsByVariable(factorsContainingVariable, variableToEliminate)
            # The second element of the result is the joined factor itself
            joinedFactor = joinedFactorResult[1]

            # Eliminate the variable from the joined factor,
            # but only if there are other unconditioned variables remaining
            if len(joinedFactor.unconditionedVariables()) > 1:
                factorAfterElimination = eliminate(joinedFactor, variableToEliminate)
                remainingFactors.append(factorAfterElimination)

            # Update our list of factors given evidence to the list of remaining factors for the next iteration
            factorsGivenEvidence = remainingFactors

        # Join any remaining factors
        finalJoinedFactor = joinFactors(factorsGivenEvidence)
        # Normalize the final joined factor
        normalizedFinalFactor = normalize(finalJoinedFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not self:  # Check if distribution is empty
            return None

        total = sum(self.values())
        if total == 0:  # Check if all values are zero
            return None
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for eachPossibleGhostPosition in self.beliefs:
            probOfNoisyGivenTruePos = self.getObservationProb(noisyDistance, pacmanPosition,
                                                                           eachPossibleGhostPosition, jailPosition)
            self.beliefs[eachPossibleGhostPosition] *= probOfNoisyGivenTruePos
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            # Get the new position distribution for the ghost given its old position
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            #Use modulo operator to cycle through the list of legal positions and assign each particle to a position.
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            #if isinstance(particle, list):  # Convert particle to tuple if it's a list
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        if weights.total() == 0:
            ...
            weights.normalize()
            # Resample from the belief distribution to create a new particle list.
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            # Check for the distribution over new positions for the
            # ghost, given its previous position (oldPos)
            newPosDist = self.getPositionDistribution(gameState, oldPos)

            # Sample a new position from the distribution and add it to the list of new particles
            newParticles.append(newPosDist.sample())

        # Update the particles with the new list of sampled positions
