...
def sentence1() -> Expr:
    ...
    combinedExpr = conjoin([expr1, expr2, expr3])
    return combinedExpr
...
def sentence3() -> Expr:
    ...
    combinedSentence = conjoin([sentence1, sentence2, sentence3])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    testExpr = premise & ~conclusion
    model = findModel(testExpr)
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    negatedStatement = ~inverse_statement
    return pl_true(negatedStatement, assignments)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for eachLiteral1, eachLiteral2 in itertools.combinations(literals, 2):
        # For each possible pair, construct a clause that they cannot both be true.
        clauses.append(~eachLiteral1 | ~eachLiteral2)
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    pacmanAtxyNow = PropSymbolExpr(pacman_str, x, y, time=now)

    causesDisjoined = disjoin(possible_causes) if possible_causes else None

    return pacmanAtxyNow % causesDisjoined
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        wallPresent = PropSymbolExpr(wall_str, x, y)
        pacmanNotAtxy = ~PropSymbolExpr(pacman_str, x, y, time=t)
        pacphysics_sentences.append(wallPresent >> pacmanNotAtxy)

    # 2. Pacman's Location:
    pacmanLocationExprs = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords]
    pacphysics_sentences.append(exactlyOne(pacmanLocationExprs))

    # 3. Pacman's Actions:
    actionExprs = [PropSymbolExpr(eachDirection, time=t) for eachDirection in DIRECTIONS]
    pacphysics_sentences.append(exactlyOne(actionExprs))

    # 4. Sensor Model
    if sensorModel is not None:
        sensorAxiomsExprs = sensorModel(t, non_outer_wall_coords)
        pacphysics_sentences.append(sensorAxiomsExprs)

    # 5. Successor Axioms
    if successorAxioms is not None and t>0:
        successorAxiomsExprs = successorAxioms(t, walls_grid, non_outer_wall_coords)
        pacphysics_sentences.append(successorAxiomsExprs)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Pacman's initial and action conditions
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    KB.append(PropSymbolExpr(action0, time=0))
    KB.append(PropSymbolExpr(action1, time=1))

    # Applying the pacphysics axioms without sensorModel
    KB.extend([
        pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid=walls_grid, sensorModel=None,
                         successorAxioms=allLegalSuccessorAxioms),
        pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid=walls_grid, sensorModel=None,
                         successorAxioms=allLegalSuccessorAxioms)
    ])

    # Query 1: Can Pacman be at (x1, y1) at time t=1?
    model1 = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time=1)]))

    # Query 2: Can Pacman NOT be at (x1, y1) at time t=1?
...
def positionLogicPlan(problem) -> List:
    ...
    # Add knowledge: Pacman's starting location
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))

    for t in range(50):
        print(f"Current Time step: {t}")

        # Add knowledge: Pacman can only be at exactly one of the non-wall coordinates at this timestep
        possibleLocations = [PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_wall_coords]
        KB.append(exactlyOne(possibleLocations))

        # Check for a satisfying assignment that includes reaching the goal
        goalReached = PropSymbolExpr(pacman_str, xg, yg, time=t)
        if findModel(conjoin(KB + [goalReached])):
            return extractActionSequence(findModel(conjoin(KB + [goalReached])), actions)

        # Add knowledge: Pacman takes exactly one action per timestep
        KB.append(exactlyOne([PropSymbolExpr(action, time=t) for action in actions]))

        # Transition Model: for all possible pacman positions
        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, t + 1, walls_grid))

    # If no solution is found within the timestep limit
    return []
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        # Food successor axiom for each non-wall coordinate
        for x, y in non_wall_coords:
            # initial attempt with partial misunderstanding:
            # food_eaten = PropSymbolExpr(pacman_str, x, y, time=t) & PropSymbolExpr(food_str, x, y, time=t)
            # KB.append(PropSymbolExpr(food_str, x, y, time=t + 1) % (~food_eaten))
            if (x, y) in food:
                foodPresent = PropSymbolExpr(food_str, x, y, time=t)
                pacmanHere = PropSymbolExpr(pacman_str, x, y, time=t)
                KB.append((foodPresent & ~pacmanHere) >> PropSymbolExpr(food_str, x, y, time=t+1))
                KB.append(~foodPresent >> ~PropSymbolExpr(food_str, x, y, time=t+1))

        # Check for a satisfying assignment that includes reaching the goal
        goalReached = conjoin([~PropSymbolExpr(food_str, x, y, time=t) for x, y in food])
        if findModel(conjoin(KB + [goalReached])):
            return extractActionSequence(findModel(conjoin(KB + [goalReached])), actions)
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        # Add Knowledge: Pacphysics Axioms
        KB.append(conjoin(
        ...
        # Add Knowledge: Percepts
        percepts = agent.getPercepts()
        perceptExprs = fourBitPerceptRules(t, percepts)
        KB.append(conjoin(perceptExprs))

        # Update possible pacman locations & Add Knowledge: Provable Pacman Locations
        possible_locations = []
        for x, y in non_outer_wall_coords:
            pacmanAtxy = PropSymbolExpr(pacman_str, x, y, time=t)
            #print(f"Entails Pacman is at ({x}, {y}) at time {t}: {entails(conjoin(KB), pacman_at_coord)}")
            #print(f"Entails Pacman is NOT at ({x}, {y}) at time {t}: {entails(conjoin(KB), ~pacman_at_coord)}")
            if entails(conjoin(KB), pacmanAtxy):
                possible_locations.append((x, y))
                KB.append(pacmanAtxy) # Pacman is provably at this location
            if not entails(conjoin(KB), ~pacmanAtxy):
                possible_locations.append((x, y)) # Pacman might be here
            else:
                KB.append(~pacmanAtxy) # Pacman is provably not at this location
        #print(f"Possible locations at time {t}: {possible_locations}")
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Add Knowledge: Initial Location
    ...
    for t in range(agent.num_timesteps):
        # Add Knowledge: Pacphysics Axioms
        KB.append(conjoin(
            pacphysicsAxioms(t, all_coords, non_outer_wall_coords, problem.walls, sensorAxioms, allLegalSuccessorAxioms)))
        ...
        # Update Provable Wall locations & Add Knowledge: Provable Wall Locations
        for x, y in non_outer_wall_coords:
            wallPresent = PropSymbolExpr(wall_str, x, y)
            noWallPresent = ~wallPresent
            if entails(conjoin(KB), wallPresent):
                known_map[x][y] = 1
                KB.append(wallPresent) # Wall is provably present
            elif entails(conjoin(KB), noWallPresent):
                known_map[x][y] = 0
                KB.append(noWallPresent) # Wall is provably not present
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        # Add Knowledge: Percepts
        percepts = agent.getPercepts()
        perceptExprs= numAdjWallsPerceptRules(t, percepts)
        KB.append(conjoin(perceptExprs))

        # Add Knowledge: SLAM Sensor
        ...
        # Use updated KB to Update Provable Wall locations & Add Knowledge: Provable Wall Locations
        for x, y in non_outer_wall_coords:
            wallPresent = PropSymbolExpr(wall_str, x, y)
            noWallPresent = ~wallPresent
            #print(f"Entails Wall exists at ({x}, {y}) at time {t}: {entails(conjoin(KB), wallPresent)}")
            #print(f"Entails Wall does NOT exists at ({x}, {y}) at time {t}: {entails(conjoin(KB), noWallPresent)}")
            #if (entails(conjoin(KB), wallPresent) and entails(conjoin(KB), noWallPresent)):
                #print("CONTRADICTION!")
            if entails(conjoin(KB), wallPresent):
                known_map[x][y] = 1 # Wall is provably present
                KB.append(wallPresent)
            elif entails(conjoin(KB), noWallPresent):
                known_map[x][y] = 0 # Wall is provably not present
                KB.append(noWallPresent)
            #print(f"Possible locations at time {t}: {known_map}")

        # Use updated KB to Update possible pacman locations & Add Knowledge: Provable Pacman Locations
        possible_locations = []
        for x, y in non_outer_wall_coords:
            pacmanAtxy = PropSymbolExpr(pacman_str, x, y, time=t)
            if findModel(conjoin(KB + [pacmanAtxy])):
                possible_locations.append((x, y))
            if (entails(conjoin(KB), pacmanAtxy)):
                KB.append(pacmanAtxy)
            elif (entails(conjoin(KB), ~pacmanAtxy)):
                KB.append(~pacmanAtxy)
